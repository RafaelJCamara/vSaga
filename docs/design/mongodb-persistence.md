...
| 8 | `Persistence:Provider` seam in `Dashboard.Api` and the sample — **landed with the Redis provider, 2026-09-26** ([`redis-persistence.md`](redis-persistence.md) §8.1): both hosts switch on `Persistence:Provider` (`Postgres` default, `Redis`), the dashboard's health check is registered under the provider-neutral `"persistence"`, and the startup migration is skipped when no `DbContext` is registered. This stage becomes "add a `MongoDb` case to each switch" | Existing `VSaga.Dashboard.Api.Tests` green; `docker compose build` succeeds for both images |
